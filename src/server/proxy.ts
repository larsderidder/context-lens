...
export function createProxyHandler(
  ...
  return function handleProxy(
    ...
    if (req.method !== "POST") {
      forwardRequest(req, res, cleanUrl as url.UrlWithStringQuery, null, opts);
      return;
    }
    ...
    req.on("end", () => {
      ...
      try {
        ...
        console.log(
          `  ⚠ Body is not JSON (${bodyBuffer.length} bytes), capturing raw`,
        );
        ...
        store.storeRequest(
          ...
          undefined,
          undefined,
          selectHeaders(req.headers),
        );
        forwardRequest(
          req,
          res,
          cleanUrl as url.UrlWithStringQuery,
          bodyBuffer,
          opts,
        );
        return;
      ...
      const proxyReq = protocol.request(
        ...
        (proxyRes) => {
          ...
          const capturedResHeaders = selectHeaders(
          ...
          proxyRes.on("end", () => {
            ...
            if (!isUtilityEndpoint) {
              if (isStreaming) {
                store.storeRequest(
                  contextInfo,
                  { streaming: true, chunks: respBody },
                  source,
                  bodyData,
                  meta,
                  capturedReqHeaders,
