...
function makeStore(opts?: Partial<{ maxSessions: number }>): { store: Store; dir: string; cleanup: () => void } {
...
describe('Store', () => {
  it('stores, compacts, and increments revision', async () => {
    ...
    const body = {
      ...
      system: 'be helpful',
      messages: [{ role: 'user', content: 'hello' }, { role: 'assistant', content: 'hi' }, { role: 'user', content: 'more' }],
    };
    const ci = parseContextInfo('anthropic', body, 'anthropic-messages');
    const resp: ResponseData = { model: 'claude-sonnet-4-20250514', stop_reason: 'end_turn', usage: { input_tokens: 1, output_tokens: 2 } } as any;

    const rev0 = store.getRevision();
    const entry = store.storeRequest(ci, resp, 'claude', body, { httpStatus: 200 }, { 'user-agent': 'claude-cli/1.0' });
  ...
  it('evicts oldest conversations when maxSessions is exceeded', async () => {
    ...
    const body1 = { model: 'claude-sonnet-4', metadata: { user_id: 'session_11111111-1111-1111-1111-111111111111' }, messages: [{ role: 'user', content: 'one' }] };
    const ci1 = parseContextInfo('anthropic', body1, 'anthropic-messages');
    store.storeRequest(ci1, { model: 'claude-sonnet-4', stop_reason: 'end_turn', usage: { input_tokens: 1, output_tokens: 1 } } as any, 'claude', body1);

    // Ensure timestamps differ.
    await new Promise(r => setTimeout(r, 5));

    const body2 = { model: 'claude-sonnet-4', metadata: { user_id: 'session_22222222-2222-2222-2222-222222222222' }, messages: [{ role: 'user', content: 'two' }] };
    const ci2 = parseContextInfo('anthropic', body2, 'anthropic-messages');
    store.storeRequest(ci2, { model: 'claude-sonnet-4', stop_reason: 'end_turn', usage: { input_tokens: 1, output_tokens: 1 } } as any, 'claude', body2);
  ...
  it('persists state and restores compact entries via loadState()', () => {
    ...
    const body = { model: 'claude-sonnet-4', metadata: { user_id: 'session_33333333-3333-3333-3333-333333333333' }, messages: [{ role: 'user', content: 'hi' }] };
    const ci = parseContextInfo('anthropic', body, 'anthropic-messages');
    store.storeRequest(ci, { model: 'claude-sonnet-4', stop_reason: 'end_turn', usage: { input_tokens: 1, output_tokens: 1 } } as any, 'claude', body);
